...
mp3 などの録音済み音声ファイルから文字起こしと要約を行う後フェーズでは、Apple 公式フレームワークのみを使う。調査結果と availability 分岐方針は [Task/31-apple-official-transcription-availability.md](Task/31-apple-official-transcription-availability.md) に記録する。

- 文字起こしは `SpeechAnalyzer` / `SpeechTranscriber` を優先し、非対応環境ではオンデバイス対応の `SFSpeechRecognizer` を検討する。
- 要約と構造化抽出は `FoundationModels` / `LanguageModelSession` を使う。
- macOS 26 専用APIは狭いadapterに隔離し、現行アプリ全体のdeployment targetはすぐには引き上げない。
- オンデバイス処理を前提にし、外部STT/LLMや追加モデル重みはこの経路に含めない。
